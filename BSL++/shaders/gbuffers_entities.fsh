...
void main(){
	...
	if (albedo.a > 0.0){
		...
		float NdotL = clamp(dot(normal,lightVec)*1.05-0.05,0.0,1.0);
		...
		#ifdef Desaturation
		float desat = clamp(max(sqrt(sqrt(length(fullshading/3)))*skymap,skymap)*sunVisibility*(1-rainStrength*0.4) + sqrt(torchmap) + float(entityColor.a > 0.05) + emissive, DesaturationFactor * 0.3, 1.0);
