...
void main(){
    //Material Flag
    float foliage = float(matches(mat, foliage_mat));
    float emissive = float(matches(mat, emissive_mat)) * EmissiveBrightness;
    float lava = float(matches(mat, lava_mat));
	...
	if (albedo.a > 0.0){
        ...
        albedo.rgb = pow(albedo.rgb, vec3(1.0 + 0.5 * wetness * skymap * skymap * skymap * skymap));
